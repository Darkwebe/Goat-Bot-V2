...
module.exports = {
  ...
  onChat: async function ({ event, api, threadsData, usersData }) {
    ...
    if (event.type === 'message_unsend') {
      ...
      if (getMsg.attachment[0] === undefined) {
        api.sendMessage(`${name}  à essayer de nous cachers un putain de truc mon ami(e)pas de secret ok?ಥ‿ಥ>: ${getMsg.msgBody}`, threadID);
      } else {
        let num = 0;
        let msg = {
  ...
  onStart: async function ({ api, event, threadsData, args }) {
    ...
    if (args.length > 0 && (args[0] === 'on' || args[0] === 'off')) {
      ...
      return api.sendMessage(`Successfully turned ${resendEnabled ? 'Allumeron✔' : 'éteint ✔off'} resend!`, threadID, messageID);
    }
